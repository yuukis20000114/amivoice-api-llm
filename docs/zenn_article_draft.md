---
title: "AmiVoice + Claude で音声認識の「惜しい誤り」を直す — 6,000件で検証してみた"
emoji: "🎤"
type: "tech"
...
| | テキスト |
|---|---------|
| 正解 | ポンテベドラ県の県都はポンテベドラである |
| ASR結果 | アンテベートラッキーのKENTはポンテベドラである。 |
| 訂正後 | ポンテベドラ・ラッキーのKENTはポンテベドラである。 |

<!-- textlint-enable -->

「アンテベートラッキー」→「ポンテベドラ県」は音の対応関係が崩壊していて、LLMにも復元できません。
...
| 項目 | コスト |
|------|--------|
| AmiVoice API（約8.6時間分の音声） | 約680円 |
| Claude Sonnet API（6,197回の訂正リクエスト） | 約1,730円（$11.2） |
| **合計** | **約2,410円** |

1件あたり約0.39円です。悪化ケース対策のフィルタを入れれば、Claude APIの呼び出し回数が減ってさらに安くなります。
