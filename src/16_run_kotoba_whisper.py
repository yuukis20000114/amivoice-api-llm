#!/usr/bin/env python3
"""kotoba-whisper-v2.0 で音声認識を実行し CSV を出力する.

Phase1 ASR スクリプト.

実行例:
    run_gpu uv run python src/16_run_kotoba_whisper.py --variant clean
    run_gpu uv run python src/16_run_kotoba_whisper.py --variant white_snr_10
"""
...
DEFAULT_RAW_BASE = Path("outputs/raw_responses/kotoba_whisper")
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output is None:
        args.output = DEFAULT_OUTPUT_DIR / f"asr_kotoba_whisper_{args.variant}.csv"
...
def main() -> int:  # noqa: C901, PLR0912, PLR0915
    ...
    # Split pending into valid (audio exists) and error (missing)
