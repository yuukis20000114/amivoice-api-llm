#!/usr/bin/env python3
"""Whisper large-v3-turbo で音声認識を実行し CSV を出力する.

Phase1 ASR スクリプト.

実行例:
    run_gpu uv run python src/15_run_whisper_large_v3_turbo.py --variant clean
    run_gpu uv run python src/15_run_whisper_large_v3_turbo.py --variant white_snr_10
"""
...
DEFAULT_RAW_BASE = Path("outputs/raw_responses/whisper_large_v3_turbo")
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output is None:
        args.output = (
            DEFAULT_OUTPUT_DIR / f"asr_whisper_large_v3_turbo_{args.variant}.csv"
        )
